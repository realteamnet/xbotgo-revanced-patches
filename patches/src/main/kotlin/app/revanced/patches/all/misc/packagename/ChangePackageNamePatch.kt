...
/**
 * Universal "Change package name" patch, ported verbatim from
 * ReVanced/revanced-patches (app.revanced.patches.all.misc.packagename) so it
 * can live alongside the XbotGo tablet patch. Self-contained: the upstream
 * `app.revanced.util` helpers (getNode / asSequence) are inlined below so this
 * fork does not need to vendor the whole util package.
 */

private fun NodeList.asSequence() = (0 until length).asSequence().map(::item)
